...
router.post('/update',function(req,res){
  if(req.body && req.body.recipe){
    let recipeReq = req.body.recipe;
    Recipe.update({ _id: recipeReq._id }, { $set: { name: recipeReq.name,recipe_country_id:recipeReq.country._id,recipe_category_id:recipeReq.category._id,remarks:recipeReq.remarks ,time:recipeReq.time,people:recipeReq.people,  recipe_difficulty_id:recipeReq.difficulty._id}}, function(err,recipe){
      if(err){
        jsonAnswer = {"data":null,"error":Config.ERROR_DB};
        res.send(jsonAnswer);
      }
...
router.get('/get', function (req, res) {
  Recipe.find({}).sort({name: 1}).populate('recipe_country_id').populate('recipe_category_id').populate('recipe_difficulty_id').exec(function(err,recipes){
    if(err){
